...
# ✅ 토익 정보 추출 함수
def extract_info_from_toeic(name, text):
    """
    ...
    if match_registration:
        ...
        result["토익_수험번호"].append(registration_number)

    # 🔹 (2) 토익 발급번호 (6자리-10자리)
    pattern_certificate = r"(\d{6}-\d{10})"
    match_certificate = re.findall(pattern_certificate, text)
    if match_certificate:
        result["토익_발급번호"].append(match_certificate[0])  # 첫 번째 값만 저장

    # 🔹 (3) 생년월일 (가장 오래된 날짜 찾기)
    earliest_birth_date = extract_earliest_birth_date(text)
    if earliest_birth_date:
        result["토익_생년월일"].append(earliest_birth_date)

    return result

# # ✅ 입력 및 출력 파일 경로
# input_file = "./toeic_input.xlsx"
# output_file = "./test_toeic_output.xlsx"

# # ✅ 엑셀 파일 읽기
# df = pd.read_excel(input_file)

# # ✅ 결과를 저장할 데이터프레임 생성 (출력 컬럼 맞추기)
# output_columns = ["토익_수험번호", "토익_발급번호", "토익_생년월일"]
...
#     name = 1
#     # ✅ 토익 추출 함수 실행
#     extracted_data = extract_info_from_toeic(name, text)

#     # ✅ 결과 데이터프레임에 추가
#     new_row = pd.DataFrame({
#         "파일명": file_name,
#         "토익_수험번호": [", ".join(extracted_data["토익_수험번호"]) if extracted_data["토익_수험번호"] else ""],
#         "토익_발급번호": [", ".join(extracted_data["토익_발급번호"]) if extracted_data["토익_발급번호"] else ""],
#         "토익_생년월일": [", ".join(extracted_data["토익_생년월일"]) if extracted_data["토익_생년월일"] else ""]
#     })
...
# print(f"✅ 토익 추출 테스트 완료! 결과가 {output_file}에 저장되었습니다.")
