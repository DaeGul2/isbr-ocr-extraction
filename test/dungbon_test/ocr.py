...
CLOVA_SECRET_KEY = "a0h2R1p3TFJOWUxqWXl3VGJLcFVsc3F1UmpJYkdtalU="  # API Secret Key

# ✅ OCR 처리할 이미지 폴더 경로
image_folder_path = './dungbon_image_data'
image_files = [f for f in os.listdir(image_folder_path) if os.path.isfile(os.path.join(image_folder_path, f))]

# ✅ 결과를 저장할 데이터프레임 생성
results_df = pd.DataFrame(columns=['파일명', 'ocr_text'])
...
for image_file in image_files:
    ...
    # OCR 결과를 데이터프레임에 추가
    new_row = pd.DataFrame({'파일명': [image_file], 'ocr_text': [extracted_text]})
    results_df = pd.concat([results_df, new_row], ignore_index=True)

# ✅ 결과 데이터프레임을 엑셀 파일로 저장
os.makedirs('./전처리텍스트_naver', exist_ok=True)
results_df.to_excel('./전처리텍스트_naver/output.xlsx', index=False)

print("✅ OCR 처리 완료 및 output.xlsx 저장 완료!")
